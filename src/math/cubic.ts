...
export function sigmaLocDerivs(k: number, c: CubicCoeffs): [number, number, number] {
 ...
 */
export function singlePieceLog1pArg(k: number, c: CubicCoeffs, n: number): number {
  const s0 = c.sigma;
 ...
 */
export function sigma2PolyCoeffsFromCubic(c: CubicCoeffs, scale: number, n: number): number[] {
  ...
  for (let i = 0; i < n; i++) fPoly[i] = sd1[i] + sd2[i] - 0.5 * aLogSqOverSig[i];
  ...
  for (let i = 0; i < n; i++) rPoly[i] = -3 * term1[i] + 0.125 * w5[i] + term3[i];
