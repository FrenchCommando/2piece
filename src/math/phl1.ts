...
import { singlePieceLog1pArg, sigmaLoc, N_TERMS, type CubicCoeffs } from './cubic';
...
export function sigma1(k: number, c: CubicCoeffs, scale: number): number {
  ...
  if (Math.abs(k) >= SIGMA_1_POLY_THRESHOLD) {
    ...
    const arg = (sigma0Total * sigmaLocTotal - ivHmTotal * ivHmTotal) / (ivHmTotal * ivHmTotal);
...
export function phl1Curve(ks: number[], c: CubicCoeffs, scale: number): number[] {
