...
import { type CubicCoeffs, sigmaLoc } from "../math/cubic";
import { knotSpikeGhlow2cc, knotSpikePhl1 } from "../math/kernel";
import { computeCurves, type ModelInputs } from "../math/model";
import { findOrThrow } from "../util";

const EX = (examples as { examples: (ModelInputs & { id: string })[] })
	.examples;
const knot = findOrThrow(
	EX,
	(e) => (e as unknown as { id: string }).id === "knot",
	"no knot example",
);
...
const cap = Math.abs(
	(cubic.sigma ** 3 * cubic.beta * knot.delta) / (20 * scale ** 4),
);
console.log(`sigmaTotal=${sigmaTotal.toFixed(6)}  scale=${scale.toFixed(3)}`);
console.log(`delta=${knot.delta}`);
console.log(
	`σ_2 clip = |Δσ_2(0)| = ${cap.toExponential(3)} ann% (= ${(cap * 100).toFixed(3)} bps)`,
);
console.log();
console.log(
	`x (=k/σ_total)   k            σ_loc(k+, perturbed)   universal-spike(ann%)   extended-spike(ann%)`,
);
