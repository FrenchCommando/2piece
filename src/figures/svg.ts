...
function niceAxis(min: number, max: number, maxTicks = 6) {
  ...
  const ticks: number[] = [];
  for (let v = lo; v <= hi + 0.5 * step; v += step) ticks.push(Math.round(v / step) * step);
  const decimals = Math.max(0, -Math.floor(Math.log10(step)));
  return { lo, hi, ticks, decimals };
}

function panel(series: Series[], o: PanelOpts, yTop: number): string {
  const live = series.filter((s) => s.y.some((v) => isFinite(v)));
  let xMin = Infinity;
  let xMax = -Infinity;
  let yMin = Infinity;
  let yMax = -Infinity;
  ...
  for (const xv of xA.ticks) {
    ...
    const xx = px(xv);
    parts.push(`<line x1="${xx}" y1="${y1}" x2="${xx}" y2="${y0}" stroke="#ececf0"/>`);
    parts.push(
      `<text x="${xx}" y="${y0 + 16}" text-anchor="middle" font-size="11" fill="#52525b">${xv.toFixed(xA.decimals)}</text>`,
    );
  }
  if (o.zeroLine)
    parts.push(`<line x1="${x0}" y1="${py(0)}" x2="${x1}" y2="${py(0)}" stroke="#71717a"/>`);
...
export function renderSvg(panels: { series: Series[]; opts: PanelOpts }[]): string {
  const H = PANEL_H * panels.length;
  const body = panels.map((p, i) => panel(p.series, p.opts, i * PANEL_H)).join('\n');
