...
export function drawChart(canvas: HTMLCanvasElement, series: Series[], opts: ChartOpts): void {
  ...
  for (const s of live)
    for (let i = 0; i < s.x.length; i++) {
      const xv = s.x[i];
      const yv = s.y[i];
      if (xv < xMin || xv > xMax || !isFinite(yv)) continue;
      if (yv < yMin) yMin = yv;
      if (yv > yMax) yMax = yv;
    }
  ...
  const yAxis = niceAxis(opts.yDomain ? opts.yDomain[0] : yMin, opts.yDomain ? opts.yDomain[1] : yMax);
  if (opts.yDomain) {
