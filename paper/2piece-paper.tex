...
constants. The constant is the $C^{2}$, log-normal-IV analog of the
universal $C^{0}$ coefficient $\tfrac{1}{16}\sqrt{\pi/2}$ that
Costeanu and Pirjol obtained in normal IV --- two members of the same
Duhamel-perturbation family, at adjacent regularity classes, on
adjacent powers of $T$. This makes ``piecewise-cubic with an at-the-money knot, mapped
...
correction is a parameter-free Brownian-bridge kernel; the at-the-money
restriction is what keeps it bounded. In doing so it answers, for the
$C^{2}$ regularity class in log-normal Black IV, the open question
Costeanu and Pirjol \citep{costeanuPirjol2011} left in their \S5.25
after settling the $C^{0}$, normal-IV case. The remainder derives the
correction and validates it against the Dupire forward PDE
\citep{dupire1994}.
...
\paragraph{Direct prior art: knot residuals at ATM.} The closest
predecessor is Costeanu--Pirjol \citep{costeanuPirjol2011}, who treat
the analogous problem one regularity class harsher: a $C^{0}$ local
vol with a jump in $\sigma'_{\mathrm{loc}}$ at the forward, in
\emph{normal} (Bachelier) implied vol. They prove that the ATM normal
IV picks up a non-analytic $\sqrt{T}$ term with universal coefficient
$\tfrac{1}{16}\sqrt{\pi/2}\approx 0.0784$ proportional to the jump,
derived by Duhamel perturbation against a constant-$\sigma$ Black
kernel and validated against an exact solution. Their \S5.25 leaves
explicitly open whether higher-power-of-$T$ non-analytic terms appear
for higher-smoothness-class jumps. The present paper answers that
question for the $C^{2}$ class (the natural smoothness of a
piecewise-cubic local vol, where only $\sigma'''_{\mathrm{loc}}=\gamma$
jumps) in \emph{log-normal} Black IV: the residual is at order
$T^{3/2}$ (carried by the factor $\sigma_{\mathrm{total}}^{3}$), the
kernel is a one-dimensional Brownian-bridge integral
$\Phi_{BB}(x,0)$, and its universal peak constant is
$3\sqrt{2\pi}/128\approx 0.0588$. The two coefficients sit side by side
as the $C^{0}$ and $C^{2}$ ATM-knot residual constants of the same
Duhamel--perturbation lineage.
...
Beta-function moment. The peak $3\sqrt{2\pi}/128$ answers the
$C^{2}$, log-normal case of the open question Costeanu and Pirjol left
in their \S5.25 after settling the $C^{0}$, normal case at
$\tfrac{1}{16}\sqrt{\pi/2}$; extending the bounded form to $w\neq0$,
and to the second-order $\Delta\gamma$ term, is the natural next step.
