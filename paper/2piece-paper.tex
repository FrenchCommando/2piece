...
\end{equation}
We use the suffix ``c'' (lower-case) throughout for ``the closed-form
knot correction appropriate to this baseline''; thus PHL1c, and later
GHLOW2c.
...
The figure also shows GHLOW2c --- the $T^{2}$-baseline analogue of
PHL1c. Off the knot it tracks the PDE more tightly than PHL1c
(capturing the smooth $\sigma_2 T^2$ term PHL1 omits). Naively stacking
the PHL1 kernel $\Phi_{BB}^{\mathrm{dir}}$ on top of GHLOW2 would leave
an analytic value jump at $k=0$ that we close in closed form below.

\paragraph{The $\sigma_2$ value jump and its repair.}
The naive PHL1-style directed kernel subtracts only BBF0's and
$\sigma_1$'s $\delta$-variations from $\Phi_{BB}$. GHLOW2 carries one
...
gives the GHLOW2-directed kernel
\begin{equation}
...
\label{eq:ghlow2-dir}
\end{equation}
In parallel with the PHL1c equation, the corrected GHLOW2 method is
\begin{equation}
...
is exact to first order. The new subtraction is no longer a universal
$x$-only function as the PHL1 kernel's was; it carries the unperturbed
cubic's $(b,a,g)$ parametrically, with $b/20$ at $x=0$ matching
\eqref{eq:ghlow2-gap} exactly. It remains bounded for the same $w=0$
reason --- no bridge-integral offset, no diverging polynomial --- and is
evaluated directly through the same $\sigma_2$ closed forms already used
for the GHLOW2 baseline. Figure~\ref{fig:knot} shows GHLOW2c with this
extended kernel: the residual at $k=0$ collapses to numerical noise
($\sim10^{-4}$~bps, more than three orders below the PHL1-kernel residual), and
GHLOW2c now tracks the PDE more tightly than PHL1c across the band
while remaining value-continuous at the knot.
...
\caption{Unhappy case: a fake at-the-money knot. Six curves on each
panel, with the asymptotic order in $T$ at the knot:
...
\textbf{GHLOW2} (teal, $T^{2}$: PHL1 $+$ $\sigma_2 T^{2}$, smooth-cubic
accuracy raised one order but carrying an analytic value jump at $k=0$
\eqref{eq:ghlow2-gap} of $-0.171$~bps for this preset, because $\sigma_2(0)$
depends on $\sloc'''(0) = 6\gamma$ and $\gamma$ jumps by $\delta$ at the
knot);
\textbf{GHLOW2c} (orange, $T^{2}+T^{3/2}$: GHLOW2 $+$ the extended
directed kernel \eqref{eq:ghlow2-dir}, which subtracts $\sigma_2$'s
$\delta$-variation in addition to the BBF0 and $\sigma_1$ pieces and
cancels the value jump while remaining bounded for the same $w=0$ reason).
The error panel (bps vs PDE) makes the ladder concrete: BBF0
$\gtrsim 60$~bps off, PHL1 a few bps off with a knot kink, PHL1c and
GHLOW2 within $\sim 1$~bp (each missing something different --- PHL1c
the smooth-cubic $\sigma_2 T^{2}$ term, GHLOW2 the knot residual), and
GHLOW2c collapsing to numerical noise.}
\label{fig:knot}
\end{figure}
...
collapse extends naturally to the $T^{2}$ baseline: subtracting
$\sigma_2$'s $\delta$-variation in \eqref{eq:ghlow2-dir} closes a
$\gamma$-linear value jump in $\sigma_2$ given in closed form by
\eqref{eq:ghlow2-gap}, and the GHLOW2c method that results is
value-continuous at the knot while inheriting the smooth-cubic
$\sigma_2 T^2$ accuracy off it. The remaining natural extension is the
