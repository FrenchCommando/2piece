...
\paragraph{Notation.} The small-$T$ heat-kernel expansion of the
local-vol transition density (Berestycki--Busca--Florent
\citep{berestycki2002}, Henry-Labord\`ere \citep{henrylabordere2008},
Gatheral et al.\ \citep{gatheral2012}) yields a closed-form expansion
of the implied volatility, $\siv \approx \sigma_0 + \sigma_1\,T +
\sigma_2\,T^{2} + \cdots$, with each $\sigma_n$ a closed-form
functional of $\sloc$. Under our $T=1$ convention
(§\ref{sec:setup-lv}), the $T^{n}$ factors are absorbed into the
$\sigma_n$ symbols and the expansion reads $\siv \approx \sigma_0 +
\sigma_1 + \sigma_2 + \cdots$. The three baselines of this paper are
the successive truncations:
\textbf{BBF0} $=\sigma_0$,
