...
lineage. The kernel is the one-dimensional Brownian-bridge integral
$\Phi_{BB}(x,0)$ derived in Section~3 below.
...
at the natural expansion point and lies outside that scope. De~Marco's
...
(the object PHL1c closes) actually is.
...
\label{eq:phibb-dir}
\end{equation}
We will refer to \eqref{eq:phibb-dir} as the \emph{universal} directed
...
\midrule
universal kernel  & $\Phi_{BB}^{\mathrm{dir}}(x,0)$, eq.~\eqref{eq:phibb-dir} & both sides; pure function of $x$ \\
extension (piece) & the $\sigma_2$ $\delta$-variation subtraction in eq.~\eqref{eq:ghlow2-dir} & $x>0$ only; $(b,a,g)$-parametric \\
extended kernel   & $\Phi_{BB,\,\mathrm{GHLOW2}}^{\mathrm{dir}}$, eq.~\eqref{eq:ghlow2-dir} & both sides; $=$ universal kernel $+$ extension \\
\bottomrule
\end{tabular}
...
$\Phi_{BB}^{\mathrm{dir}}$ \eqref{eq:phibb-dir} on the GHLOW2 baseline:
\begin{equation}
...
The universal directed kernel \eqref{eq:phibb-dir} subtracts only BBF0's
and $\sigma_1$'s $\delta$-variations from $\Phi_{BB}$. GHLOW2 carries one
...
diverging polynomial in $\Phi_{BB}$ --- so the jump is a closed scalar.
...
\begin{equation}
\Phi_{BB,\,\mathrm{GHLOW2}}^{\mathrm{dir}}(x,0)
= \Phi_{BB}(x,0)
  - \tfrac{x^3}{4}\Hev(x)
  - \tfrac{x}{4}\Hev(x)
...
\eqref{eq:phibb-dir} for $x>0$; the fourth, $(b,a,g)$-parametric piece
...
\siv^{\mathrm{GHLOW2cc}}(k)=\siv^{\mathrm{GHLOW2}}(k)
   +\delta\cdot\stot^{3}\cdot\Phi_{BB,\,\mathrm{GHLOW2}}^{\mathrm{dir}}(k/\stot,0),
\label{eq:ghlow2cc}
\end{equation}
...
extended kernel collapses back to the universal $\Phi_{BB}^{\mathrm{dir}}$
...
GHLOW2, $\Phi_{BB}$), the correction for a strike $k$ depends on the
...
Brownian-bridge kernel $\Phi_{BB}^{\mathrm{dir}}$, repairing the slope
...
kernel $\Phi_{BB}^{\mathrm{dir}}$ \eqref{eq:phibb-dir}, equivalently
...
$\delta\,\stot^{3}\,\Phi_{BB}^{\mathrm{dir}}(k/\stot,0)$, exactly the
gap PHL1c adds to PHL1 \emph{and} the gap GHLOW2c adds to GHLOW2 (both
identical, sharing the $\Phi_{BB}(0,0)=3\sqrt{2\pi}/128$ peak and
...
$3\sqrt{2\pi}/128$ and no fitted constants. After subtracting the
